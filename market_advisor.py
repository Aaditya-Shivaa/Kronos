...
# Load environment variables from .env (no external dependency needed)
...
def generate_local_explanation(symbol_name, current, support, resistance, ret_pct, trend_text):
    # Short-circuit when no API key is configured (avoids wasted 401 round-trips)
    if not OPENROUTER_API_KEY:
        return _fallback_explanation(symbol_name, support, resistance, ret_pct)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"[WARN] OpenRouter API returned {response.status_code}: {response.text[:200]}")
    except Exception as e:
        print(f"[WARN] OpenRouter request failed: {e}")

    # Clean beginner fallback explanation if OpenRouter service is unavailable
    return _fallback_explanation(symbol_name, support, resistance, ret_pct)


def _fallback_explanation(symbol_name, support, resistance, ret_pct):
    if ret_pct > 0:
...
def main():
    ...
    print("[EXEC] Analyzing Indian equities and currency benchmarks with OpenRouter AI commentary...")
